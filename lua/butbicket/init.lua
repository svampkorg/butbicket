...
local function set_groups()
	...
	for _, name in ipairs({
		...
		groups = vim.tbl_extend("force", groups, require("butbicket.integrations." .. name).highlights())
	end
...
function theme.colorscheme()
	if vim.fn.has("nvim-0.8") == 0 then
		vim.notify("Neovim 0.8+ is required for butbicket colorscheme", vim.log.levels.ERROR, { title = "butbicket" })
		return
	end
