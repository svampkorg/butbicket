...
--
-- The module is split for readability: `util` (shared primitives), `sample`
-- (knob model + example content), `serialize` (flavour <-> Lua text), and
-- `color_editor` (the OKLch `c` editor). This file is the orchestrator: panel
-- rendering, interaction, lifecycle, and `open`/`close`.

local color_editor = require("butbicket.playground.color_editor")
...
local serialize = require("butbicket.playground.serialize")
...
local make_float = util.make_float
local graded_palette = util.graded_palette

local default_opts = serialize.default_opts

local M = {}
M.serialize = serialize.serialize -- exposed for tests
M.serialize_variants = serialize.serialize_variants -- exposed for tests
...
local function accept(session)
  local block = "flavour = " .. serialize.serialize_variants(session.variants)
...
---Open the flavour playground (or focus it if already open). Builds the panel +
---example floats, seeds the knobs from the active `config.flavour`, and starts
---the live preview. No-op beyond focusing when a session already exists.
function M.open()
  if P then
  ...
  for i, d in ipairs(sample.DIFF_DEMO) do
  ...
  for i, d in ipairs(sample.DIAG_DEMO) do
